...
if ps x | grep "bot_plus"|grep -v grep 1>/dev/null 2>/dev/null; then
   clear
   echo -e "\033[1;32mO BOT ADMINISTRADOR DROPLET YA ESTA ACTIVO !\033[0m"
   sleep 3
   bot_telegram
 else
   clear
   echo -e "\033[1;32mINICIANDO BOT ADMINISTRADOR DROPLET... \033[0m"
   sleep 3
   cd /etc/SSHPlus
...
echo ""
 if ps x | grep "bot_plus"|grep -v grep 1>/dev/null 2>/dev/null; then
   clear
   echo -e "\033[1;32mPARANDO BOT ADMINISTRADOR DROPLET ... \033[0m"
   sleep 3
   pidbot_plus=$(ps x|grep "bot_plus"|awk -F "pts" {'print $1'})
   kill -9 $pidbot_plus 1>/dev/null 2>/dev/null
   screen -wipe 1>/dev/null 2>/dev/null
   echo ""
   echo -e "\033[1;32m[ \033[1;31mBOT ADMINISTRADOR DROPLET PARADO! \033[1;32m]\033[0m"
   sleep 3
   bot_telegram
 else
   echo ""
   echo -e "\033[1;32mO BOT ADMINISTRADOR DROPLET 1.2 YA ESTA DESACTIVADO!\033[0m"
   sleep 3
   bot_telegram
 fi
}
...
reset_bot () {
	echo -e "\033[1;32mRESETANDO BOT ADMINISTRADOR DROPLET 1.2... \033[0m"
	if ps x | grep "bot_plus"|grep -v grep 1>/dev/null 2>/dev/null; then
		pidbot_plus=$(ps x|grep "bot_plus"|awk -F "pts" {'print $1'})
	 	kill -9 $pidbot_plus 1>/dev/null 2>/dev/null
	 	screen -wipe 1>/dev/null 2>/dev/null
...
echo -e "\E[44;1;37m          BOT TELEGRAM ADMINISTRADOR DROPLET 1.2            \E[0m"
echo ""
...
{
echo -e "\E[44;1;37m               INSTALADOR BOT ADMINISTRADOR DROPLET 1.2                 \E[0m"
echo ""
echo -e "                 \033[1;33m● \033[1;31mATENCAO \033[1;33m●\033[0m"
echo ""
echo -e "\033[1;32m●\033[1;33m Saiba que o BOT ADMINISTRADOR DROPLET 1.2 ainda possui opcoes um pouco\033[0m"
...
if [[ "$resposta" = 's' ]]; then
	echo ""
  	...
  	cd $HOME
      echo ""
      echo -e "\033[1;31m[\033[1;32m BOT ADMINISTRADOR DROPLET 1.2 ATIVADO \033[1;31m]\033[0m"
      sleep 2
      clear
  	bot_telegram
  else
